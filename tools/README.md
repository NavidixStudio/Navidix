...
## هشدار: چهار سازنده‌ی درس از خروجی‌شان عقب‌ترند

`build-lesson-systems.py` را اجرا نکن مگر اینکه اول درستش کنی. اگر اجرا شود،
از `ai-systems.html` لینک «قسمت بعد → اولین کارِ واقعی» را **پاک می‌کند**،
چون قسمت ۵ بعد از نوشته‌شدن سازنده دستی اضافه شده و به سازنده نرسیده. همین
حالت برای هر چهار `build-lesson-*.py` محتمل است: خروجی کامیت‌شده جلوتر از
سازنده است، پس ساختنِ دوباره یک قدم به عقب است نه به جلو.

با یک اجرای آزمایشی و `git diff` معلوم شد. قاعده‌ی امن تا وقتی سازنده‌ها
به‌روز نشده‌اند: **اول اجرا کن، بعد `git diff` را بخوان، و تا وقتی diff را
نفهمیده‌ای کامیت نکن.** اگر diff چیزی را حذف می‌کند، سازنده عقب است نه صفحه.
