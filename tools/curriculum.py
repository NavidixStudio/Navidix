...
EXTRA_CSS = '''
/* ---- the journey ---- */
.jrn{ max-width:760px; margin:0 auto; }
.jrn__stats{ display:grid; grid-template-columns:repeat(auto-fit,minmax(128px,1fr));
  gap:10px; margin:26px 0 34px; }
.jrn__stat{ background:var(--nvx-pane,rgba(20,24,31,.7)); border:1px solid var(--nvx-line,#262A31);
...
.lrow{ display:flex; align-items:center; gap:12px; padding:13px 15px; border-radius:11px;
  background:rgba(16,19,25,.6); border:1px solid #1E232B; text-decoration:none;
...
{chr(10).join(stages)}
  ...
  <p class="jrn__note" id="jnote">پیشرفتت همین حالا روی همین مرورگر ذخیره می‌شود و
  ثبت‌نام نمی‌خواهد. به‌زودی می‌توانی یک حساب بسازی تا همین پیشرفت روی گوشی و
  کامپیوترت یکی شود.</p>
